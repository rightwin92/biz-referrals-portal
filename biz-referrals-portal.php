 ...
 * Description: Front-end portal for Ask / Requirement / Give / Lead / Response with moderation, scheduling, sharing, CTA (Like/Enquiry counters), dashboard, and front-page override.
 * Version: 1.4.3
 * Author: BRP Team
 * License: GPL2+
 */
...
define('BRP_VER','1.4.3');
define('BRP_PATH', plugin_dir_path(__FILE__));
define('BRP_URL',  plugin_dir_url(__FILE__));

/** ---------- SAFETY LAYER ---------- */
if (!function_exists('brp_safe_require')) {
  function brp_safe_require($rel){
    $abs = BRP_PATH . ltrim($rel,'/');
    if (file_exists($abs)) { require_once $abs; return true; }
    error_log('[BRP] Missing include: '.$abs);
    add_action('admin_notices', function() use ($rel){
      echo '<div class="notice notice-error"><p><strong>Biz Referrals Portal:</strong> Missing file <code>'.esc_html($rel).'</code>. Please update/reinstall the plugin.</p></div>';
    });
    return false;
  }
}

/** Counter helpers (used by CTA) */
if (!function_exists('brp_get_count')) {
  function brp_get_count($post_id, $key){ $v=(int)get_post_meta($post_id,$key,true); return max(0,$v); }
}
if (!function_exists('brp_inc_count_once')) {
  function brp_inc_count_once($post_id, $key){ $v=brp_get_count($post_id,$key)+1; update_post_meta($post_id,$key,$v); return $v; }
}

/** Load includes (guarded; no fatal if missing) */
...
brp_safe_require('includes/frontpage-override.php'); // guarded override
...
/** Allow WEBP on older WP */
add_filter('upload_mimes', function($m){ $m['webp']='image/webp'; return $m; });

/** Visibility: Active + Start/End window */
if (!function_exists('brp_is_active_and_in_window')) {
  ...
  if (is_singular(['ask','requirement','give','lead','response'])){
    ...
    if(!brp_is_active_and_in_window($post->ID)){
      if(current_user_can('edit_post',$post->ID) || current_user_can('manage_options')){
        add_filter('the_content', fn($c)=>'<div class="brp-notice">This post is paused or outside its schedule. Only you and admins can see it.</div>'.$c);
      }else{ global $wp_query; $wp_query->set_404(); status_header(404); }
    }
  }
});

/** Helper: build CTA HTML (Like + Enquiry) */
if (!function_exists('brp_build_cta_html')) {
...
/** Auto-unpublish after End Date */
add_action('brp_hourly_cron', function(){
  $types=['ask','requirement','give','lead','response'];
  $q = new WP_Query(['post_type'=>$types,'post_status'=>'publish','posts_per_page'=>200,'fields'=>'ids','no_found_rows'=>true,'meta_query'=>[['key'=>'_brp_end','compare'=>'EXISTS']]]);
  $now=current_time('timestamp');
  foreach($q->posts as $pid){
    $end=get_post_meta($pid,'_brp_end',true);
    if($end && ($ts=strtotime($end)) && $now>$ts) wp_update_post(['ID'=>$pid,'post_status'=>'draft']);
  }
});

/** Reminder email 24h before End Date */
add_action('save_post', function($post_id,$post,$update){
  if (!in_array($post->post_type,['ask','requirement','give','lead','response'],true)) return;
  $end=get_post_meta($post_id,'_brp_end',true); if(!$end) return;
  $when=strtotime($end)-DAY_IN_SECONDS; if($when<=current_time('timestamp')) return;
  $args=[$post_id];
...
add_action('brp_send_end_reminder', function($post_id){
  ...
  $to=[]; $author=get_userdata($post->post_author);
  if($author && is_email($author->user_email)) $to[]=$author->user_email;
  $submitter=get_post_meta($post_id,'_brp_email',true); if($submitter && is_email($submitter)) $to[]=$submitter;
  if(!$to) return;
  $site=wp_specialchars_decode(get_bloginfo('name'),ENT_QUOTES);
  $sub='Reminder: Your post ends soon – '.$post->post_title;
